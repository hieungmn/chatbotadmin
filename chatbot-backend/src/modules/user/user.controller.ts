...
export const getAllAdminUsers = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        // Truy vấn đích danh bảng public.admin_users có cột role
...
export const createNewAdminUser = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        // Chèn chuẩn xác cấu trúc bảng public.admin_users công ty giao
        await pool.query(
        ...
        // Ghi lại nhật ký hành động vào bảng public.admin_audit_logs theo chuẩn Schema V2
        try {
...
// 4. API Lấy danh sách các site_id hiện tại của một user (Để Frontend hiển thị trạng thái Checkbox ban đầu)
export const getUserPermissions = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        
        const queryStr = 'SELECT site_id FROM public.user_site_permissions WHERE user_id = $1';
        const result = await pool.query(queryStr, [id]);
        
        // Trích xuất dữ liệu thành mảng chuỗi đơn giản: ['s-wing', 'c-wing']
        const siteIds = result.rows.map(row => row.site_id);
        
        return res.json({ success: true, site_ids: siteIds });
    } catch (error) {
        console.error('❌ LỖI LẤY QUYỀN TRANG CỦA USER:', error);
        return res.status(500).json({ success: false, message: 'Lỗi máy chủ khi lấy quyền nhân viên!' });
    }
};

// 5. API Bật/Tắt quyền phân phối đơn lẻ của từng Site (Đã chỉnh sửa để tương thích cấu trúc lưu mảng từ form)
export const toggleSitePermission = async (req: Request, res: Response) => {
    try {
        const { userId, siteId, isChecked } = req.body;
