...
export default function WidgetSandbox() {
    ...
    const handleSendMessage = async (forcedText?: string) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (!botReply || botReply.trim() === "" || botReply.includes("該当する質問") || botReply.includes("見つかりませんでした")) {
                ...
                setMessages(prev => [
                    ...prev,
                    {
                        ...
                        sender: 'bot',
                        text: botReply,
                        faq_id: response.data.faq_id || undefined,
                        showFeedback: !!response.data.faq_id
                    }
                ]);
            }
        } catch (error) {
    ...
    const handleFeedback = async (msgId: string, faqId: number, score: number) => {
        ...
        try {
            const http = (axios as any).default || axios;
            await http.post('http://localhost:3000/api/v1/chat/feedback', { site_id: siteId, faq_id: faqId, score: score });
        } catch (e) {
            console.error(e);
        }
    };
        ...
        <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif' }}>
            ...
            {showFullBot && (
                <div style={{ position: 'fixed', bottom: '24px', right: '24px', zIndex: 99999 }}>
                    ...
                    {/* LAUNCHER ĐA NĂNG ĐÃ TĂNG KÍCH THƯỚC & CÂN BẰNG TỶ LỆ */}
                    {!isOpen && (
                        <div
                        ...
                        >
                            ...
                            >
                                {/* ✕ Nút xám tắt thanh dài (Bên trái chữ) */}
                                <span
                                    onClick={(e) => {
                                        e.stopPropagation();
                    ...
                    {/* KHUNG HỘP THOẠI CHAT CHÍNH (KHI BẤM MỞ) */}
                    {isOpen && (
                        <div style={{
                            ...
                            <div ref={chatBodyRef} className="chat-body-scroll" style={{ flex: 1, padding: '20px', overflowY: 'auto', background: '#F8FAFC', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                                {messages.map((msg) => (
                                    <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                                        ...
                                        {/* TÍCH HỢP LẠI CẶP NÚT LIKE DISLIKE BO GÓC MƯỢT MÀ */}
                                        {msg.sender === 'bot' && msg.showFeedback && msg.faq_id && (
                                            <div
                                                style={{
                                                    display: 'flex',
                                                    gap: '8px',
